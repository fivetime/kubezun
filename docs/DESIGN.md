...
**配额估算(2026-08-14,采外部评审的公式)**:`secgroup_rules` 配额按
**Σ(每条规则的 peers × ports × 地址族)** 估,不按策略规则条数——一条 SG rule 只能载
一个 remote、一个端口区间(`acl.py:263` 一条 rule = 一条 ACL),折叠不经 Neutron API
不可达(`ruleset.go:13-17` 自陈)。10 peer × 5 端口的一条规则 = 100 条 rule,
"放开几个数量级"要按这个乘积放。
...
**⚠️ 分辨性补测(同日,被"观察到全云更新了吗"一问逼出来的——第一版结论有一半是错的)**:
用 `inc-engine/show-stats` 快照计数器 + SB lflow 总数,逐操作 diff:

| 操作 | lflow 引擎 | SB lflow 总数 |
|---|---|---|
| 空 SG 建/删 | **full recompute +1~2** | 2219→2219(**不变**) |
| 端口加/减组 | **full recompute +1** | 2219→2225(+6,ACL 物化) |
| 地址组加地址 | **引擎未触发**(+0/+0) | 不变 |

**三条结论(更正版)**:

1. **northd 对 SG 建删/端口组变更每次都是全量 lflow 重算**——§13 引用的全云重算路径
   **机制上被证实,不是过时**。8→1000 组耗时平坦的真正原因:**空 PG 贡献零 lflow**,
   重算的对象没变多。⚠️ **伸缩变量是全云 lflow 总量,不是组数**:实验床 2.2k 条 lflow
   → ~20ms;线性外推 100k 条(中型云)→ **~1s/次策略事件**,这才是 region 容量规划
   该用的数。策略建删频率 × 全量重算耗时 = northd 的预算线。
2. **ovn-controller 侧观测代价 ≤1ms**(flow-generation,样本非零)——本次 delta 只有
   6 条 lflow,**分辨不出它是增量还是小全量**,§13 对 controller 侧的判断维持存疑、
   按保守假设规划。
3. **address group 轴真免费**——northd 引擎完全未触发,SB 无变化。
   "peer churn 压到地址组"是全设计里唯一被无保留证实的押注。

**第三次补测(同日,决定性)**:pod 建删——端口**带着 knp 组出生/消亡**——
`lflow: full+0 inc+6`、`northd: full+0 inc+12`、`port_group: full+0 inc+6`:
**纯增量**。至此事件成本矩阵完整:

| 事件类 | 规模下频率 | northd 路径 |
|---|---|---|
| pod 建删(端口携组) | 极高(churn) | **增量** ✓ |
| peer 地址更新 | 极高 | **免费**(引擎几乎不醒) ✓ |
| 策略建删(SG 对象) | 人类频率 | **全量**,O(全云 lflow) |
| 存量端口改组列表(标签/策略编辑) | 低 | **全量** |

**热路径全在增量/免费侧,全量重算只在人类频率的事件上**——这正是本设计押的形状,
三次测量后完整证实。

**对设计的含义**:每策略一 SG 的对象经济学依旧正确;但"平坦"不是赦免——
**策略事件的成本随整个 region 的 lflow 体量线性走**,所以 §7.4.2 的 region 切分逻辑
反而被加强了:lflow 体量(≈ ACL × 有成员的 datapath)就是 region 的容量预算轴。
未证:真实端口规模下的实测斜率(实验床 ~30 端口撑不起这条曲线)。
