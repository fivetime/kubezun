...
- [x] **（P2 → 已做）外部评审 (b):Neutron 写全部挪出 pod 热路径（2026-08-14 实现并实测）**：
      ...
      **测试的妙处**:夹具的 Neutron 本来就是 nil——**nil 即零调用证明**,旧代码在这两个
      ...
      封闭白名单形状的脆弱性,不换
- [x] ⭐ **压 OVN 那两个数 —— 已测(2026-08-14),两根轴都比恐惧的好**：
      northd 对 SG 建删 **8→1000 组完全平坦**(~20ms);controller 对端口组变更
      **≤1ms 且确认执行**(增量路径在,§13 那句"无 delta 路径"对当前版本过时);
      address group 两侧 **0ms**。数字表与方法进了 DESIGN §7.7.7。
      ⚠️ 未证:端口数轴(实验床仅 ~30 端口)、万级策略弯折点——上真环境才有。
      ⚠️ devstack 同机,绝对值仅量级参考。原条目:（DESIGN §7.7.7，实验床 `ovn-appctl stopwatch/show`）：
