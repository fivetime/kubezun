...
| `/root/k8s-zun-provider/openstack/openstack-ceilometer` | OpenStack 计量服务(fork) | **计费方案的事实依据,别猜**。⚠️ **2026-08-11 核实:走原生 ceilometer 给 Zun 计费,现在两端都是空的**——① ceilometer 默认监听的 exchange(`ceilometer/notification.py:67-71`)是 nova/glance/neutron/cinder/heat/keystone/trove/zaqar/swift/ceilometer/magnum/dns/ironic/aodh,**没有 zun**;`ceilometer/data/meters.d/meters.yaml` 无任何 zun/container 计量项,事件定义同样为零。② **Zun 根本不发计量通知**:全仓库唯一 notifier 用法在 `zun/common/exception.py:56-84`(异常包装器),且 `wrap_exception(notifier=...)` 无任何调用点传值=死代码;无 versioned notification 对象,无生命周期事件,`zun/conf` 无 notification 配置组。⇒ 要落地需两边都补(Zun 发通知 + ceilometer 加 exchange/meter),路径同 magnum。**配额则是现成的**:`zun/common/quota.py` + `quotas`/`quota_usages` 表 + `/v1/quotas`,按 project 记 containers/cpu/memory/disk,capsule 与 container 天然同账。**替代路线**:我们已有 per-container 用量(fork `c6f85263` capsule stats + kubezun `/metrics/resource`),计费系统若能吃 Prometheus 格式则比补 ceilometer 短得多——取决于账单从哪出 |
...
| `/root/cloud-provider-openstack` | K8s 官方 OpenStack cloud provider（**go 1.26 + gophercloud v2.12.0，与 kubezun 目标栈同基线，可 drop-in**；Apache 2.0） | **阶段 4 直用**：`pkg/kms/barbican/` + `cmd/barbican-kms-plugin/`（etcd KMS 加密后端，DESIGN §8.1 现成实现）。**查证/参考**：`pkg/client/`（AuthOpts→ServiceClient 构造范式）；`pkg/csi/cinder/`（PVC→Cinder 供给流程参考，DESIGN §14.8；⚠️ 其 `cloud.conf` 静态多 cloud 凭据模型需换 per-tenant appcred——kubetron 已记录同一问题）；`pkg/csi/manila/`（RWX share 挂载编排——Zun fork Manila/virtiofs 项的规格来源，§8.2）；**卷扩容的两段式范式**：`pkg/csi/cinder/controllerserver.go:984-1050`（extend + 等 available/in-use + 已够大则幂等返回）与 `nodeserver.go:377-410`（`RescanBlockDeviceGeometry` → `NewResizeFs().Resize`），⚠️ 对照 `pkg/csi/manila/nodeserver.go:361-363` **node 侧直接 `Unimplemented`**——NFS 的文件系统归文件服务器，这是"Manila 扩容几乎白送、Cinder 才是难的那个"的直接依据；`pkg/openstack/loadbalancer.go`（Octavia batch member reconcile `buildBatchUpdateMemberOpts`——kubetron service reconciler 的上游范式，一般不直接用）；`cmd/k8s-keystone-auth/`（Keystone↔K8s authn/authz webhook——若未来租户身份统一到 Keystone 的候选，现阶段 kubezoo 走证书/SA token，仅备查）；`cmd/octavia-ingress-controller/`（L7 ingress 参考——⚠️ OVN provider 无 L7，需 amphora，阶段 4 后再议） |
| `/root/k8s-zun-provider/openstack/openstack-nova-incus` | **nova 的 incus 计算驱动（自研）**——实现了**完整的 nova 存储契约**，复杂度高于我们要做的任何一处，因此是"这件事标准做法长什么样"的最好样本 | ⚠️ **查它,不要猜**。`nova/virt/incus/driver.py:12808-12828`（`extend_volume`：**os-brick 的 connector 自带 `extend_volume()`**，逐连接类型实现 rescan 并返回新尺寸——Python 侧白送，CPO 手写 rescan 只是因为它是 Go；**且校验 fail-closed**，os-brick 报的尺寸小于请求就抛 `VolumeExtendFailed`，不假定成功）；`:6459` `supports_extend_volume=True` 能力声明。⚠️ **nova 不做 `resize2fs`**——它把裸块设备交给 VM、文件系统归 guest 长；而 Zun 在**计算节点上**挂文件系统（`zun/volume/driver.py:507-533`），所以我们**多一步长文件系统**，那一步照 CPO 的形状。Manila 挂载硬化（`nosuid,nodev`/mount 超时/挂载校验/per-share 锁）已抄进 fork（`447a3a59`）；**未抄**：share journal（挂载 crash 恢复）、CephFS secretfile（per-share 凭据模板）。另有 ceilometer 补丁，是"给非 nova 服务补计量"的现成范式 |
